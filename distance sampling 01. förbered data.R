...
rip_perp <-ripor_analys %>% 
  dplyr::select(Sample.Label, distance, period, weather)
...
plot(hn.df.cos)
no.covar.fit <- ddf.gof(hn.df.cos$ddf, main = "Half-normal detection function, no covariates") #plottar qq-plot. Syns tydligt att den blev sämre
